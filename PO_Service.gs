...
// ⚡ INI DIA FUNGSI YANG SEMPAT TERHAPUS (UNTUK TOMBOL NEXT/PREV LOG PO)
function getPODashboardPage(page) {
  try { return JSON.stringify(getPODashboardData(page)); } 
  catch (e) { return JSON.stringify({ error: e.message }); }
}
...
// ─────────────────────────────────────────────────────────────────────────
// 1. AMBIL POOL PO YANG MASIH 'OPEN' (MONITORING JADWAL SUPPLIER)
// ─────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
// 2. AMBIL DATA HISTORI KESELURUHAN PO (DASHBOARD LOG)
// ─────────────────────────────────────────────────────────────────────────
function getPODashboardData(page) {
  var pageSize = 30;
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  for (var i = data.length - 1; i >= 1; i--) {
    if (!data[i][h.indexOf('PO_No')]) continue;
    
    var rawTgl = data[i][h.indexOf('PO_Date')];
    var tglStr = rawTgl instanceof Date ? Utilities.formatDate(rawTgl, Session.getScriptTimeZone(), 'dd MMM yy') : String(rawTgl);
    var rawSch = data[i][h.indexOf('Schedule')];
    var schStr = rawSch instanceof Date ? Utilities.formatDate(rawSch, Session.getScriptTimeZone(), 'dd MMM yy') : String(rawSch);

    allData.push({
      po_date: tglStr,
      po_no: String(data[i][h.indexOf('PO_No')]).trim(),
      vendor: String(data[i][h.indexOf('Vendor')]).trim(),
      schedule: schStr,
      item_code: String(data[i][h.indexOf('Item_Code')]).trim(),
      po_q: data[i][h.indexOf('PO_Q')] || 0,
      po_kg: parseFloat(data[i][h.indexOf('PO_KG')]) || 0,
      status: String(data[i][h.indexOf('STATUS')]).trim().toUpperCase(),
      owner: String(data[i][h.indexOf('Owner')] || '').trim()
    });
  }
...
// ─────────────────────────────────────────────────────────────────────────
// 3. SIMPAN INPUT PO BARU (DENGAN TARGET_LOC)
// ─────────────────────────────────────────────────────────────────────────
